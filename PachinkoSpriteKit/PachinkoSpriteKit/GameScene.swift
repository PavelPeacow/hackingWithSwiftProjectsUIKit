...
//  PachinkoSpriteKit
//
//  Created by Павел Кай on 13.08.2022.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var ballsLimit = 5 {
        didSet {
            ballsLabel.text = "Balls: \(ballsLimit)"
        }
    }
        ...
        if let touch = touches.first {
            ...
            if objects.contains(editLabel) {
                ...
                if editingMode {
                    ...
                    addChild(box)
                } else if ballsLimit != 0 {
                    let arrayBalls = ["ballRed", "ballBlue", "ballYellow", "ballGrey", "ballPurple", "ballGreen"]
                    let ball = SKSpriteNode(imageNamed: arrayBalls.randomElement()!)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if nodeA.name == "ball" {
            ...
            collisionBox(ball: nodeA, object: nodeB)
        } else if nodeB.name == "ball" {
            collisionBetween(ball: nodeB, object: nodeA)
            collisionBox(ball: nodeB, object: nodeA)
        }
    }
    ...
    func collisionBox(ball: SKNode, object: SKNode) {
        if object.name == "box" {
            destroy(ball: object)
        }
    }
